...
public class RedstoneClampBlock extends AbstractRedstoneComparisonBlock {
    ...
    public RedstoneClampBlock(FabricBlockSettings settings) {
        super(settings);
        this.setDefaultState(this.stateManager.getDefaultState().with(CLAMP, 0).with(POWERED, false));
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        var filter = PeculiarHelper.clampLoop(0, 15, state.get(CLAMP) + (player.isSneaking() ? -1 : 1));
