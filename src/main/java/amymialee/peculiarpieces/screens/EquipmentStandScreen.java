...
public class EquipmentStandScreen extends HandledScreen<EquipmentStandScreenHandler> {
    private static final Identifier TEXTURE = PeculiarPieces.id("textures/gui/six_inventory.png");
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        if (handler.getSlot(5).getStack().isEmpty()) {
            RenderSystem.setShaderTexture(0, EMPTY_OFFHAND);
            DrawableHelper.drawTexture(matrices, x + 53, y + 20, getZOffset(), 0, 0, 16, 16, 16, 16);
        }
        ...
        if (handler.getSlot(2).getStack().isEmpty()) {
            ...
            DrawableHelper.drawTexture(matrices, x + 107, y + 20, getZOffset(), 0, 0, 16, 16, 16, 16);
        }
        if (handler.getSlot(3).getStack().isEmpty()) {
            RenderSystem.setShaderTexture(0, EMPTY_HEAD);
            DrawableHelper.drawTexture(matrices, x + 125, y + 20, getZOffset(), 0, 0, 16, 16, 16, 16);
        }
    }
}